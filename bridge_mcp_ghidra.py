...
def _fetch_tool_definitions(server_url: str) -> list[dict]:
    """Fetch tool schemas from the Ghidra plugin's /mcp/tools endpoint."""
    try:
        resp = requests.get(f"{server_url}/mcp/tools", timeout=DEFAULT_TIMEOUT)
...
def _call_tool(server_url: str, tool_def: dict, arguments: dict) -> tuple[str, dict | None, bool]:
...
async def main():
    ...
    tools = _fetch_tool_definitions(ghidra_server_url)
    if not tools:
    ...
    async def call_tool(name: str, arguments: dict) -> CallToolResult:
        ...
        display_text, structured_data, is_error = await asyncio.to_thread(
            _call_tool, ghidra_server_url, tool_def, arguments or {}
        )
